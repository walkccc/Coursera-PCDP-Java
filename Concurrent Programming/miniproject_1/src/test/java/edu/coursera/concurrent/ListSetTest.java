...
public class ListSetTest extends TestCase {
  ...
  private static void printStats(TestResults ref, TestResults test, final SequenceGenerator seq,
    ...
    System.out.println(
        (test.addRate / ref.addRate) + "x improvement in add throughput (" + ref.addRate + " -> " + test.addRate + ")");
    System.out.println((test.containsRate / ref.containsRate) + "x improvement in contains throughput ("
        + ref.containsRate + " -> " + test.containsRate + ")");
    System.out.println((test.removeRate / ref.removeRate) + "x improvement in remove throughput (" + ref.removeRate
        + " -> " + test.removeRate + ")");
    System.out.println("=========================================================");
  }

  public void testCoarseGrainedLockingRandomLarge() throws InterruptedException {
    final SequenceGenerator addSeq = new RandomSequenceGenerator(0, getNCores() * randNumsLength, randNumRange);
    final SequenceGenerator containsSeq = new RandomSequenceGenerator(1, getNCores() * randNumsLength, randNumRange);
  ...
  public void testCoarseGrainedLockingRepeatingLarge() throws InterruptedException {
    final SequenceGenerator addSeq = new RepeatingSequenceGenerator(getNCores() * 6 * randNumsLength, randNumsLength);
    final SequenceGenerator containsSeq = new RepeatingSequenceGenerator(getNCores() * 6 * randNumsLength,
        randNumsLength);
    final SequenceGenerator removeSeq = new ReversedSequenceGenerator(
  ...
  public void testReadWriteLocksRandomLarge() throws InterruptedException {
    ...
    testReadWriteLocksHelper(addSeq, containsSeq, removeSeq, expectedAdd, expectedContains, expectedRemove, "Large");
  }

  public void testReadWriteLocksRandomSmall() throws InterruptedException {
    final SequenceGenerator addSeq = new RandomSequenceGenerator(0, getNCores() * randNumsLength / 2, randNumRange);
    final SequenceGenerator containsSeq = new RandomSequenceGenerator(1, getNCores() * randNumsLength / 2,
        randNumRange);
    final SequenceGenerator removeSeq = new ReversedSequenceGenerator(
    ...
    testReadWriteLocksHelper(addSeq, containsSeq, removeSeq, expectedAdd, expectedContains, expectedRemove, "Small");
  }
  ...
  public void testReadWriteLocksRepeatingSmall() throws InterruptedException {
    final SequenceGenerator addSeq = new RepeatingSequenceGenerator(getNCores() * 3 * randNumsLength, randNumsLength);
    final SequenceGenerator containsSeq = new RepeatingSequenceGenerator(getNCores() * 3 * randNumsLength,
        randNumsLength);
    final SequenceGenerator removeSeq = new ReversedSequenceGenerator(
  ...
  private void testCoarseGrainedLockingHelper(final SequenceGenerator addSeq, final SequenceGenerator containsSeq,
      final SequenceGenerator removeSeq, final double expectedAdd, final double expectedContains,
      final double expectedRemove, final String datasetName) throws InterruptedException {

    final TestResultsPair results = runKernel(() -> new CoarseList(), "CoarseList", () -> new SyncList(), "SyncList",
    ...
    final String addmsg = String.format(
        "Expected add throughput to remain " + "similar (at least %fx) with locks, but found %fx", expectedAdd,
        addImprovement);
    assertTrue(addmsg, addImprovement >= expectedAdd);

    final String containsmsg = String.format(
        "Expected contains throughput to " + "remain similar (at least %fx) with locks, but found %fx",
        expectedContains, containsImprovement);
    assertTrue(containsmsg, containsImprovement >= expectedContains);

    final String removemsg = String.format(
        "Expected remove throughput to " + "remain similar (at least %fx) with locks, but found %fx", expectedRemove,
        removeImprovement);
    assertTrue(removemsg, removeImprovement >= expectedRemove);
  }

  public void testReadWriteLocksHelper(final SequenceGenerator addSeq, final SequenceGenerator containsSeq,
      final SequenceGenerator removeSeq, final double expectedAdd, final double expectedContains,
      final double expectedRemove, final String datasetName) throws InterruptedException {
    final TestResultsPair results = runKernel(() -> new RWCoarseList(), "RWCoarseList", () -> new SyncList(),
        "SyncList", addSeq, containsSeq, removeSeq);
    ...
    final String addmsg = String.format(
        "Expected add throughput " + "improvement to be at least %fx with read-write locks, but " + "found %fx",
        expectedAdd, addImprovement);
    assertTrue(addmsg, addImprovement >= expectedAdd);

    final String containsmsg = String.format(
        "Expected contains throughput " + "improvement to be at least %fx with read-write locks, but " + "found %fx",
        expectedContains, containsImprovement);
    assertTrue(containsmsg, containsImprovement >= expectedContains);

    final String removemsg = String.format(
        "Expected remove throughput " + "improvement to be at least %fx with read-write locks, but " + "found %fx",
        expectedRemove, removeImprovement);
    assertTrue(removemsg, removeImprovement >= expectedRemove);
  }

  private static TestResultsPair runKernel(final ListFactory factoryA, final String lblA, final ListFactory factoryB,
      final String lblB, final SequenceGenerator addSeq, final SequenceGenerator containsSeq,
     ...
     * scheduling. We interleave runs of the baseline and the implementation we're
     * testing to try to make any interference on Coursera's autograding platform
  ...
  private static TestResults mainKernel(final int numThreads, final ListSet list, final String lbl,
      final SequenceGenerator addSeq, final SequenceGenerator containsSeq, final SequenceGenerator removeSeq)
    ...
    /*
     * Assert that the resulting list is sorted and save the length of the list
     * after the initial adds.
     */
    int listLengthAfterAdds = 1;
    Entry prev = list.getHead();
    Entry curr = prev.next;
    while (curr != null) {
      assertTrue("List was not sorted, index " + (listLengthAfterAdds - 1) + " is " + prev.object.intValue()
          + " and index " + listLengthAfterAdds + " is " + curr.object.intValue(),
    ...
    /*
     * Verify list is still sorted and record length, successes, and failures.
     */
    int listLengthAfterRemoves = 1;
    prev = list.getHead();
    curr = prev.next;
    while (curr != null) {
      assertTrue("List was not sorted", curr.object.intValue() > prev.object.intValue());
    ...
    return new TestResults(lbl, addRate, containsRate, removeRate, listLengthAfterAdds, totalContainsSuccesses,
        totalContainsFailures, listLengthAfterRemoves, totalRemovesSuccesses, totalRemovesFailures);
  }
  ...
  private static class TestResults {
    ...
    public TestResults(final String lbl, final double addRate, final double containsRate, final double removeRate,
        final int listLengthAfterAdds, final int totalContainsSuccesses, final int totalContainsFailures,
        final int listLengthAfterRemoves, final int totalRemovesSuccesses, final int totalRemovesFailures) {
      this.lbl = lbl;
