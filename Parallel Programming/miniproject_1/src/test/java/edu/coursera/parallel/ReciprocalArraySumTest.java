...
public class ReciprocalArraySumTest extends TestCase {
  ...
  /**
   * A reference implementation of seqArraysum, in case the one in the main source
   * file is accidentally modified.
   *
   ...
   * @param useManyTaskVersion Switch between two-task and many-task versions of
   *                           the code
  ...
  private double parTestHelper(final int N, final boolean useManyTaskVersion, final int ntasks) {
    ...
    final String errMsg = String.format(
        "Mismatch in result for N = %d, expected = %f, computed = %f, absolute " + "error = %f", N, correct, sum, err);
  ...
  /**
   * Test that the two-task parallel implementation properly computes the results
  ...
  /**
   * Test that the two-task parallel implementation properly computes the results
   * for a hundred million-element array.
   */
  ...
  /**
   * Test that the many-task parallel implementation properly computes the results
   * for a million-element array.
   */
