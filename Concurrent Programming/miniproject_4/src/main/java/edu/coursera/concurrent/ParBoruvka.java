...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
  ...
  public void computeBoruvka(final Queue<ParComponent> nodesLoaded, final SolutionToBoruvka<ParComponent> solution) {
    ...
    while ((component = nodesLoaded.poll()) != null) {
      ...
      final Edge<ParComponent> minEdge = component.getMinEdge();
      if (minEdge == null) {
        solution.setSolution(component);
        break;
      }

      final ParComponent other = minEdge.getOther(component);
      if (other.isDead) {
  ...
  /**
   * ParComponent represents a single component in the graph. A component may be a
   * singleton representing a single node in the graph, or may be the result of
   * collapsing edges to form a component from multiple nodes.
   */
  public static final class ParComponent extends Component<ParComponent> {
    ...
    /**
     * List of edges attached to this component, sorted by weight from least to
     * greatest.
     */
    private List<Edge<ParComponent>> edges = new ArrayList<>();

    /**
     * The weight this component accounts for. A component gains weight when it is
     * merged with another component across an edge with a certain weight.
     */
